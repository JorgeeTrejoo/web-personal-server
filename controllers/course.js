...
function addCourse(req, res){
    ...
    //Primero recogemos los datos del body porque el id,titulo,etc vendran en el body
    const body = req.body;
    ...
    course.save((err, courseStored) => {
        if(err){
            res.status(400).send({ code: 400, message: "Este curso ya existe" });
        }else{
            if(!courseStored){
    ...
    Course.find()
            ...
            .exec((err, coursesStored) => {
                if(err){
                    ...
                    if(!coursesStored){
                        res.status(400).send({ code: 400, message: "No hay ningún curso" });
                    }else{
